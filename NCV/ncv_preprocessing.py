...
sites = len(data["Primary site"].unique()) # 44
hists = len(data["Primary histology"].unique()) # 113
hist_subs = len(data["Histology subtype 1"].unique()) # 518
zygosity = len(data["zygosity"].unique()) # 3
wt_seq = len(data["WT_SEQ"].unique()) # 4
mut_seq = len(data["MUT_SEQ"].unique()) # 4
